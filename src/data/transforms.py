...
GLOBAL_CONFIG["ToImageTensor"]=GLOBAL_CONFIG["ToImage"]
class ConvertDtype(T.ToDtype):
    def __init__(self,**k):
        super().__init__(dtype=__import__("torch").float32,scale=True)
ConvertDtype=register(ConvertDtype)
SanitizeBoundingBox = register(T.SanitizeBoundingBoxes)
GLOBAL_CONFIG["SanitizeBoundingBox"]=GLOBAL_CONFIG["SanitizeBoundingBoxes"]
        ...
        if ops is not None:
            for op in ops:
                if isinstance(op, dict):
                    name = op.pop('type')
                    transfom = getattr(GLOBAL_CONFIG[name]['_pymodule'], {'SanitizeBoundingBox':'SanitizeBoundingBoxes','ToImageTensor':'ToImage','ConvertDtype':'ConvertDtype'}.get(name,name))(**op)

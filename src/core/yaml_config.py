...
class YAMLConfig(BaseConfig):
    def __init__(self, cfg_path: str, **kwargs) -> None:
        ...
        self.use_pixel_da = cfg.get('use_pixel_da', False)
        self.use_instance_da = cfg.get('use_instance_da', False)
    ...
    def train_dataloader_source(self, ):
        if self._train_dataloader_source is None and 'train_dataloader_source' in self.yaml_cfg:
            merge_config(self.yaml_cfg)
            self._train_dataloader_source = create('train_dataloader_source')
            self._train_dataloader_source.shuffle = self.yaml_cfg['train_dataloader_source'].get('shuffle', False)

        return self._train_dataloader_source

    @property
    def train_dataloader_target(self, ):
        if self._train_dataloader_target is None and 'train_dataloader_target' in self.yaml_cfg:
            merge_config(self.yaml_cfg)
            self._train_dataloader_target = create('train_dataloader_target')
            self._train_dataloader_target.shuffle = self.yaml_cfg['train_dataloader_target'].get('shuffle', False)
